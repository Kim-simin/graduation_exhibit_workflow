...
### 4) Detailed Department Tagging Standard (학생 작품 상세 학과명 필수 표기 규격 - 차기 작업부터 고정):
* **핵심 지침**: 대학교 카드 내 개별 출품작(`artworks`) 등록 시, 상단 배지에 포괄적/광역 학부명(예: `디자인이노베이션전공`)으로 축약 노출되지 않도록, 작품별로 **상세 학과명/세부 전공명(예: `VISUAL DESIGN과`, `INDUSTRIAL DESIGN과`, `시각디자인과`, `산업디자인과` 등)**을 반드시 추출하여 `department` 필드에 기재한다.
* **적용 시점**: 현재 기존 등록된 카드는 보존하며, **차기(다음) 대학교 카드 수집 및 리서치 작업부터 고정 적용**한다.
...
본 플랫폼 개발 환경에는 `playwright` MCP가 이미 설치되어 정상 작동 중이나, 모든 웹사이트를 브라우저로 탐색하지 않고 시스템 자원과 토큰을 최적화하기 위해 엄격한 조건 하에서만 사용한다.
...
### 1) 기본 원칙: HTTP / Document Retrieval 우선
- 정적 페이지 또는 단순 문서 retrieval이 가능한 경우: `HTTP / document retrieval` 등 가벼운 방식을 최우선한다.

### 2) Playwright 사용 조건 (다음 상황에서만 한정 사용):
다음 상황 중 하나 이상에 해당할 때만 `playwright` 도구를 활성화하여 탐색한다.
1. **JavaScript rendering**: 서버사이드가 아닌 클라이언트 브라우저 자바스크립트 실행을 통해서만 DOM이 구성되는 페이지.
2. **Dynamic content**: 비동기 AJAX / Fetch API 호출로 데이터가 동적 로드되는 페이지.
3. **Click required**: 버튼, 링크, 드롭다운을 클릭해야만 교수 정보나 과제 목록이 노출되는 경우.
4. **Tab required**: [연구실 소개], [담당 교과목], [산학 과제], [학력/경력] 등 탭 전환 인터랙션이 필요한 경우.
5. **Accordion required**: 세부 연구/과제 정보가 접혀 있는 아코디언 컴포넌트를 열어야 확인 가능한 경우.
6. **Pagination interaction**: 2페이지, 3페이지 등 페이지 번호 클릭 또는 다음 페이지 버튼을 눌러야 다음 교원이 탐색되는 경우.
7. **Infinite scroll**: 스크롤 다운 이벤트를 발생시켜야 추가 리스트가 렌더링되는 경우.
8. **SPA (Single Page Application / Client-Side Rendering)**: React, Vue, Next.js, Angular 기반 단일 페이지 애플리케이션으로 라우트 전환이 일어나는 경우.
9. **Login/session dependent public page**: 공개 페이지이나 브라우저 쿠키/스토리지 컨텍스트 유지가 필요한 경우.
10. **Content only visible after interaction**: 마우스 호버(Hover), 포커스 등 특정 인터랙션 후에만 텍스트가 가시화되는 경우.
...
연구실 / 프로젝트 탭 클릭
```
이러한 단계별 브라우저 인터랙션이 요구되는 탐색에 한하여 Playwright MCP(`browser_navigate`, `browser_click`, `browser_snapshot`)를 호출한다.
